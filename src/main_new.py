...
def optimize_output(output, label_set):
    """
    Calculate output and label set editing distance and return the label with minimum distance
    """
    ...
    if args.do_train:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if args.statistic_mode != 'True':
                ...
                # args.save(args.output_dir)
                # model.save_checkpoint(args.output_dir)
                # lora_config = model.config.lora_config if hasattr(model.config, 'lora_config') else None
                # if lora_config:
                #     # 转换为字典格式
                #     lora_config_dict = lora_config.to_dict()

                #     # 定义保存路径
                #     lora_config_path = os.path.join(args.output_dir, "lora_config.json")

                #     # 保存到文件
                #     with open(lora_config_path, 'w', encoding='utf-8') as f:
                #         f.write(json.dumps(lora_config_dict, indent=4))
                # with open(os.path.join(args.output_dir, "deepspeed_config.json"), 'w', encoding='utf-8') as f:
                #     f.write(json.dumps(deepspeed_config, indent=5))
